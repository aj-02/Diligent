...
| 27/08/26 | abapGit rejected the first ZIP — *"xml incorrect"* | Not a parse error: all six files were well-formed, UTF-8 no BOM, LF, trailing newline, and the main `PROGDIR` was shape-identical to `ovl/ztest_t001/src/ztest_t001.prog.xml`, the only serialisation in this repo proven to import here. The two things that differed from that reference were `package.devc.xml` and `TPOOL` items carrying a `KEY` | Both removed. `package.devc.xml` dropped (only needed when abapGit creates the package); `TPOOL` cut back to the report title, so the five selection texts and text symbol `B01` are **maintained by hand** — Arnav's call. The ZIP now matches the proven reference in shape. Exact abapGit message never captured, so the attribution to those two files is inference, not proof | `<TR>` |
