...
* SUM_CHAR (include RM07MLDD) aggregate quantities only - they have no
* DMBTR component - so the values are aggregated in this table.
...
*BOC By Arnav on 18/08/26
*   ZMB5B 195_BRD_FS : receipt amount for the radio button
*   "Storage Loc./Batch Stock". G_OFFSET_VALUE and G_OFFSET_CURR are
*   calculated in FORM CALCULATE_OFFSETS for every radio button.
    IF  lgbst = 'X'.
...
*BOC By Arnav on 18/08/26
*   ZMB5B 195_BRD_FS : issue amount for the radio button
*   "Storage Loc./Batch Stock" - shown with a negative sign, exactly
*   like the quantity HABEN above
    IF  lgbst = 'X'.
...
* selected from MATDOC by F1000_SELECT_MSEG_MKPF, so no additional
...
*&      BESTAENDE_BERECHNEN (include RM07MLBD_FORM_01) fills these
