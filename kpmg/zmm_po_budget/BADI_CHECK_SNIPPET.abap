...
*BOC Budget control on indirect purchase By Arnav on 19.08.2026
...
*     OPEN POINT: calendar year assumed, the FS does not say.
      lv_bud_gjahr = ls_header-bedat(4).

*     Value of the account assigned items in this document.
*     Only KNTTP = K counts, per the FS, and deleted items are ignored.
*     Multiple plants in one document are already rejected earlier in this
*     method ("MULTIPLE PLANT NOT ALLOW"), so one plant bucket is enough.
...
*         Change to type 'W' if documents should pass in this case.
...
*         The document being saved is excluded - without this a change
*         through ME22N counts its own value twice, once from the database
*         and once from the document in memory.
          SELECT SUM( p~effwr )
            FROM ekko AS k
...
*EOC Budget control on indirect purchase By Arnav on 19.08.2026
