...
*----------------------------------------------------------------------*
REPORT ZFI_RFSEPA03 MESSAGE-ID FH LINE-SIZE 80.
...
*--- BEGIN OF CHANGE BY SAP_ABAP 08.08.2026 FOR ATC ---
  SELECT SINGLE * FROM BSEG WHERE BUKRS = T_BSIS-BUKRS
                            ...
                            AND   BUZEI = T_BSIS-BUZEI.  "#EC CI_DB_OPERATION_OK[2431747]
...
* S/4: XOPVW (open item mgmt) cannot be changed by direct update in S/4HANA - use FINS_SWITCH_TO_OPEN_ITEM (Note 2745769).
...
*              AND BUZEI EQ T_BSIS-BUZEI.
  MESSAGE 'Document not updated. Open item mgmt (XOPVW) change not possible in S/4HANA - use FINS_SWITCH_TO_OPEN_ITEM. Refer SAP Note 2745769' TYPE 'E'.
