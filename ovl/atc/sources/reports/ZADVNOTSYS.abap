*--- MAIN PROGRAM: ZADVNOTSYS ---*
*&---------------------------------------------------------------------*
*& Report  ZADVNOTSYS
*&
* Title      : Advance Notification System for PO delivery date       *
*                                                                     *
* FS No.     : Final - FS-MM-Adv-Notif-Sys.docx                       *
*                                                                     *
* Author     : Amit Moza              Date : 01.02.2013               *
*                                                                     *
*                                                                     *
*                                                                     *
...
*                                                                     *
*                                                                     *
* Description: This program will intimate users through SAPMail & SMS *
*              about approaching delivery dates of material POs.      *
*                                                                     *
* Tran. Code : ZMM_ANS
***********************************************************************
* CHANGE HISTORY                                                      *
...
*                                                                     *
* 16/09/2013  CAB_SPYADAV   Changes as Per CR No.         001         *
*                           30009742                                  *
*                                                                     *
***********************************************************************
REPORT  zadvnotsys.
*********Data declaration***********
TABLES : ekpo , eket ,pa9205,
...
DATA: it_final1 LIKE STANDARD TABLE OF final,
      ...
      wa_final2 LIKE LINE OF it_final1,
      it_final2 LIKE STANDARD TABLE OF final,
      it_final3 LIKE STANDARD TABLE OF final,
      wa_final3 LIKE LINE OF it_final1,
      wa_final4 LIKE LINE OF it_final1,
      wa_final5 LIKE LINE OF it_final1.

DATA: BEGIN OF stab,
  ebeln LIKE eket-ebeln,
  txz01 TYPE txz01,
  END OF stab.
DATA: it_stab1 LIKE STANDARD TABLE OF stab,     "TABLE BEING SENT TO E-MAIL
      wa_stab1 LIKE LINE OF it_stab1,
      wa_stab2 LIKE LINE OF it_stab1.


DATA: xdate LIKE sy-datum .
DATA : it_eket1 TYPE TABLE OF eket,
       wa_eket1 TYPE eket,
       it_eket2 TYPE TABLE OF eket,
       wa_eket2 TYPE eket,
       ...
       wa_ekpo2 TYPE ekpo,
       it_cdhdr1 TYPE TABLE OF cdhdr,
       wa_cdhdr1 TYPE cdhdr.
...
sy-title = 'Advance Notification System - PO Delivery dt.'.
...
START-OF-SELECTION.
  ...
  xdate = p_date + 7 .
  SELECT * FROM eket
    INTO CORRESPONDING FIELDS OF TABLE it_eket1
    WHERE ebeln LIKE '40%'
    AND eindt = xdate .

  IF it_eket1 IS NOT INITIAL.
    SELECT * FROM ekpo
      INTO CORRESPONDING FIELDS OF TABLE it_ekpo1
      FOR ALL ENTRIES IN it_eket1
      WHERE ebeln = it_eket1-ebeln
      AND ebelp = it_eket1-ebelp
      AND loekz = ''
      AND elikz = '' .
    APPEND LINES OF it_ekpo1 TO it_ekpo2.
...
" Code Remediation changes S4 2025_1_A Conversion * *END OF CHANGE BY SAP_ABAP 05.06.2026  FOR ATC
    DELETE ADJACENT DUPLICATES FROM it_ekpo1
    COMPARING ebeln.

    LOOP AT it_ekpo1 INTO wa_ekpo1.

**taken from ZMMPOLOG***********************************************
      SELECT * FROM ekko WHERE ebeln = wa_ekpo1-ebeln.
...
**end ZMMPOLOG***********************************************
    ...
    LOOP AT it_itab1 INTO wa_itab4.
      LOOP AT it_ekpo2 INTO wa_ekpo2.
        IF wa_ekpo2-ebeln = wa_itab4-ekkey-ebeln.
          MOVE-CORRESPONDING wa_ekpo2 TO wa_final1.
          wa_final1-username = wa_itab4-username.
    ...
    ENDLOOP.
  ENDIF.
  SORT it_final1 BY username ebeln txz01.     " added  EBELN TXZ01 on 22.02.2013
  DELETE ADJACENT DUPLICATES FROM it_final1
    COMPARING username ebeln txz01 .          " added  EBELN TXZ01 on 22.02.2013
  APPEND LINES OF it_final1 TO it_final2.

******take out Unique Usernames from IT_FINAL1 into IT_FINAL2
  DELETE ADJACENT DUPLICATES FROM it_final2
    COMPARING username.

  REFRESH it_final3.

*+001 : Send email to Incharge-MM & Sub-incharge, additionally
  ...
  l_role2 = 'D:MM_PUR_PO_APPROVE_SM'.

  LOOP AT it_final2 INTO wa_final3.

    CLEAR : wa_inchrg_mm.

    MOVE-CORRESPONDING wa_final3 TO wa_inchrg_mm.

    SELECT SINGLE ekgrp FROM ekko INTO wa_inchrg_mm-ekgrp
        WHERE ebeln = wa_inchrg_mm-ebeln.
...
*    CONCATENATE '%' wa_inchrg_mm-ekgrp INTO l_role3.

*Check existing PO creater / releasing authority have purchasing
...
*  - Purchasing group
    ...
    SELECT agr_name uname FROM agr_users
            INTO CORRESPONDING FIELDS OF TABLE ist_agr_users
               WHERE  agr_name = l_role1   AND " OR agr_name = l_role2 ) AND
                      to_dat   GE sy-datum .
    ...
    IF NOT ist_agr_users_1[] IS INITIAL.
      ...
      LOOP AT ist_agr_users_1 INTO wa_agr_users.
        ...
        READ TABLE it_final2 INTO wa_final3
                        WITH KEY ebeln    = wa_inchrg_mm-ebeln
  ...
  SORT ist_inchrg_mm_s BY ebeln username.

  DELETE ADJACENT DUPLICATES FROM ist_inchrg_mm_s COMPARING ebeln username.

* Modify internal table IT_FINAL2 AND IT_FINAL1 as per above logic
...
*Internal table : IT_FINAL2
      MOVE-CORRESPONDING wa_inchrg_mm TO wa_final3.
      APPEND wa_final3 TO it_final2.

*Internal table : IT_FINAL1
      READ TABLE it_final1 INTO wa_final4
                WITH KEY ebeln    = wa_inchrg_mm-ebeln
                         username = wa_inchrg_mm-username.

      IF sy-subrc NE 0.

        APPEND wa_final3 TO it_final1.
...
****preparing final table IT_STAB1 from IT_FINAL1 and send MAIL
** and SMS correspondingly******
  LOOP AT it_final2 INTO wa_final3.
    LOOP AT it_final1 INTO wa_final4.
      IF wa_final4-username = wa_final3-username.
        APPEND wa_final4 TO it_final3.
        CLEAR: wa_final4.
      ENDIF.
    ENDLOOP.

    LOOP AT it_final3 INTO wa_final5.
      wa_stab1-ebeln = wa_final5-ebeln.
      wa_stab1-txz01 = wa_final5-txz01.
      APPEND wa_stab1 TO it_stab1.
      CLEAR: wa_stab1.
    ENDLOOP.
    SORT it_stab1 BY ebeln.

    p_email = wa_final3-username.
...
* Send SMS
    ...
    CLEAR: p_email.
    REFRESH: it_final3 , it_stab1 ,it_message ,it_attach .
  ENDLOOP.
...
FORM generate_fcat .
  ...
  ist_fcat-tabname    = 'IT_FINAL1'.
  ist_fcat-ddictxt    = 'L'.
  ist_fcat-seltext_l  = 'CPF'.
  ist_fcat-outputlen  = '20' .
  APPEND ist_fcat.
  CLEAR ist_fcat.

  ist_fcat-fieldname  = 'EBELN'.
  ist_fcat-tabname    = 'IT_FINAL1'.
  ist_fcat-ddictxt    = 'L'.
  ist_fcat-seltext_l  = 'PO No.'.
...
*             I_SCREEN_START_COLUMN = n
          TABLES
*              t_outtab           = it_final1.    "-001
               t_outtab           = it_final2.    "+001
...
FORM user_command USING ucomm TYPE sy-ucomm
  ...
  IF NOT selfield-value IS INITIAL.
    CASE ucomm.
      WHEN '&IC1'.
        READ TABLE it_final1 INDEX selfield-tabindex INTO wa_final2.
        IF sy-subrc EQ 0.
...
FORM build_xls_data_table .
  ...
  CONCATENATE 'PO No.' 'Short Desc.'
    INTO it_attach SEPARATED BY con_tab.
  CONCATENATE con_cret it_attach  INTO it_attach.
  APPEND  it_attach.

  LOOP AT it_stab1 INTO wa_stab2.
    CONCATENATE wa_stab2-ebeln wa_stab2-txz01
    INTO it_attach SEPARATED BY con_tab.
    CONCATENATE con_cret it_attach  INTO it_attach.
    APPEND  it_attach.
  ENDLOOP.
...
*        NL = CL_ABAP_CHAR_UTILITIES=>NEWLINE.
  ...
  l_text2 = crlf.
  l_text3 = 'The Advance Notification System has detected a few Purchase Orders released by you, whose delivery is scheduled on'.
  l_edd = sy-datum + 7.
...
* SENDING SMS
    ...
    l_edd = sy-datum + 7.
    CONCATENATE l_edd+6(2) l_edd+4(2) l_edd+0(4) INTO l_edd2 SEPARATED BY c.
    l_text1 = 'Delivery date of a few POs released by you is due to expire on'.
    l_text2 = l_edd2.
    l_text3 = '. List SAPMailed. '.
    l_text4 = crlf.
    l_text5 = 'ICE Team'.
    CONCATENATE l_text1 l_text2 l_text3 l_text5 INTO sms_text SEPARATED BY space.
    CONCATENATE
