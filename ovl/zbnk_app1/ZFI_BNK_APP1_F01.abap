...
*&---------------------------------------------------------------------*
*&      Form  F_RUN_PENDING_COUNT
*&---------------------------------------------------------------------*
*BOC By SAP_ABAP on 27/08/26
*  ZFI_PAYM_FILE holds one medium file per payment run (key MANDT/LAUFD/
*  LAUFI) and that single file carries every REGUT batch of the run. The
*  file can only be transmitted once EVERY batch of the run is approved at
*  both levels - sending it earlier puts the unapproved batches on the
*  wire as well.
*
*  Returns, for one run, how many of its REGUT batches still lack a
*  level-1 or a level-2 signature in ZFI_BATCH_SIGN (CV_PENDING) and how
*  many batches the run has in total (CV_TOTAL). Batches excluded by
*  F_SKIP_BATCHNO - already carrying a REGUHM-BATCHNO - are counted in
*  neither, matching what the worklists show.
*
*  Note: F_LOAD_REGUHM refills the global GT_REGUHM for this run. That is
*  safe here because PBO rebuilds the worklist, and with it GT_REGUHM,
*  before the next screen is shown.
*----------------------------------------------------------------------*
FORM f_run_pending_count USING    iv_laufd   TYPE regut-laufd
                                  iv_laufi   TYPE regut-laufi
                         CHANGING cv_pending TYPE i
                                  cv_total   TYPE i.

  TYPES: BEGIN OF ty_sign,
           batch_no    TYPE zfi_batch_sign-batch_no,
           snro        TYPE zfi_batch_sign-snro,
           digitl_sign TYPE zfi_batch_sign-digitl_sign,
         END OF ty_sign.

  DATA: lt_hdr  TYPE STANDARD TABLE OF regut,
        ls_hdr  TYPE regut,
        lt_key  TYPE STANDARD TABLE OF zfi_batch_sign-batch_no,
        lt_sign TYPE STANDARD TABLE OF ty_sign,
        lv_key  TYPE zfi_batch_sign-batch_no,
        lv_skip TYPE abap_bool.

  CLEAR : cv_pending, cv_total.
...
* REGUHM for this run, so the exclusion rule matches the worklists
  PERFORM f_load_reguhm USING lt_hdr.

* Keys of the batches that actually require approval
  LOOP AT lt_hdr INTO ls_hdr.
    CLEAR lv_skip.
    PERFORM f_skip_batchno USING ls_hdr CHANGING lv_skip.
    IF lv_skip = abap_true.
      CLEAR ls_hdr.
      CONTINUE.
    ENDIF.
  ...
  cv_total = lines( lt_key ).
  IF lt_key IS INITIAL.
    RETURN.
  ENDIF.

  SELECT batch_no, snro, digitl_sign
    FROM zfi_batch_sign
    FOR ALL ENTRIES IN @lt_key
    WHERE batch_no = @lt_key-table_line
    INTO TABLE @lt_sign.

* A batch stays pending while either approval level is still open
  LOOP AT lt_key INTO lv_key.
    READ TABLE lt_sign TRANSPORTING NO FIELDS
         WITH KEY batch_no    = lv_key
                  snro        = '1'
                  digitl_sign = 'X'.
    IF sy-subrc <> 0.
      cv_pending = cv_pending + 1.
      CONTINUE.
    ENDIF.
    READ TABLE lt_sign TRANSPORTING NO FIELDS
         WITH KEY batch_no    = lv_key
...
*BOC By SAP_ABAP on 27/08/26
* ZFI_PAYM_FILE is keyed MANDT/LAUFD/LAUFI - one medium file per payment
* run, covering every REGUT batch of that run. Filtering the worklist on
* the run-level SENT flag therefore dropped every batch of a run as soon
* as one batch had been approved and the file sent, leaving the rest
* transmitted but unapproved and invisible here.
* Approved batches are already excluded per batch below, via
* DIGITL_SIGN = ' ' AND SNRO = '2' - exactly as F_PREPARE_OP_TAB1 does for
* level 1, where this same file-level filter was removed earlier.
