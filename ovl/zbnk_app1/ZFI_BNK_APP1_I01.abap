...
* run, carrying every REGUT batch of that run. The original code below
* signed the selected batch, transmitted that file straight away and set
* SENT = 'X' on it. File and flag are both run level, so one approval put
* every batch of the run on the wire and removed them all from this
* worklist - the unselected batches went to the bank with
* DIGITL_SIGN = ' ' and could never be reached again.
*
* Now the approval is recorded for the selected batch only, and the file
* is transmitted only once every batch of the run carries both a level-1
* and a level-2 signature (FORM F_RUN_PENDING_COUNT, include
* ZFI_BNK_APP1_F01).
*
* ASSUMPTION: the signature that reaches the bank is the one produced by
* whoever approves the LAST batch of the run. With one file per run there
* can only be one, and ZFI_BATCH_SIGN has no field for a signature blob -
* only the DIGITL_SIGN flag.
*
* Old code:
*            CLEAR gs_paym2.
...
*           Record the approval for the selected batch first, so it is
*           kept even when the run is still incomplete and nothing is
*           sent. Moved up from the end of the transmission block.
            CLEAR : gs_batch_sign2.
            READ TABLE gt_batch_sign2 INTO gs_batch_sign2
                 WITH KEY batch_no = gs_final2-guid
                          signer   = sy-uname.
            IF sy-subrc <> 0.
              MESSAGE 'No open level 2 approval entry for this batch and user' TYPE 'I'.
              EXIT.
            ENDIF.
            ...
            IF sy-subrc <> 0.
              ROLLBACK WORK.
              MESSAGE 'Approval could not be saved - please retry' TYPE 'I'.
              EXIT.
            ENDIF.
            COMMIT WORK.

*           Transmit only when the whole run is approved at both levels
            CLEAR : lv_pending, lv_total, lv_done, lv_msg.
            PERFORM f_run_pending_count USING    gs_final2-laufd
                                                 gs_final2-laufi
                                        CHANGING lv_pending
                                                 lv_total.
            IF lv_pending > 0.
              lv_done  = lv_total - lv_pending.
              lv_cdone = lv_done.
              lv_ctot  = lv_total.
              CONDENSE lv_cdone.
              CONDENSE lv_ctot.
              CONCATENATE 'Batch approved.' lv_cdone 'of' lv_ctot
                          'batches in this run are approved - file not sent yet.'
            ...
            IF sy-subrc = 0.
              IF gs_paym2-sent <> ' '.
*               The run's file has already gone to the bank. The approval
*               above is still saved - this is how a batch that was sent
*               unapproved gets its signature recorded afterwards.
                MESSAGE 'Approval saved. File for this run was already sent to the bank' TYPE 'I'.
                EXIT.
              ELSE.
                proxy_data-parameters-arg0  = gs_paym2-file_name.
                proxy_data-parameters-arg1  = lv_doc_sig.
                ...
                ELSE.
                  ...
                  gs_paym2-sent_time      = sy-uzeit.
                  MODIFY zfi_paym_file FROM gs_paym2.
                  MESSAGE 'Run fully approved - file sent to the bank' TYPE 'I'.
                ENDIF.
                CLEAR : proxy_data.
              ENDIF.
            ENDIF.
