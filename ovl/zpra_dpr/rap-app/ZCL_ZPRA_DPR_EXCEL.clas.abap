  ...
  PRIVATE SECTION.
    ...
    TYPES:
      BEGIN OF ty_dpr_day,
        ...
        oil_jv          TYPE ty_qty,
        gas_jv          TYPE ty_qty,
        boepd_jv        TYPE ty_qty,
        oil_ovl         TYPE ty_qty,
        gas_ovl         TYPE ty_qty,
        boepd_ovl       TYPE ty_qty,
      END OF ty_dpr_day,
...
CLASS zcl_zpra_dpr_excel IMPLEMENTATION.
  ...
  METHOD generate_production_excel.
    ...
    LOOP AT it_data INTO DATA(ls_row).
      lv_row = |{ ls_row-production_date DATE = RAW }|
            && |\|{ ls_row-product_desc }|
            && |\|{ ls_row-asset }|
            && |\|{ ls_row-asset_desc }|
            && |\|{ ls_row-block }|
    ...
    rv_xdata = build_excel_from_rows(
      ...
      it_rows       = lt_rows
      iv_sheet_name = |DPR Production { iv_date_from DATE = RAW } to { iv_date_to DATE = RAW }|
    ).
  ENDMETHOD.
  ...
  METHOD build_excel_from_rows.
    " XCO XLSX is not available on this release -> emit a semicolon-separated
    " CSV as an xstring (UTF-8). Opens directly in Excel.
  ...
  METHOD gas_to_mmscmd.
    CASE iv_uom.
      WHEN 'MCM'.  rv_qty = iv_qty.
      WHEN 'MCF'.  rv_qty = iv_qty / '35.3'.
      WHEN 'M3'.   rv_qty = iv_qty / 1000000.
  ...
  METHOD fetch_and_export_dpr_summary.
    ...
    LOOP AT lt_days INTO ls_day.
      lv_row = |{ ls_day-production_date DATE = RAW }|
            && |\|{ ls_day-oil_jv }|
            && |\|{ ls_day-gas_jv }|
            && |\|{ ls_day-boepd_jv }|
            && |\|{ ls_day-oil_ovl }|
    ...
    rv_xdata = build_excel_from_rows(
      it_headers    = lt_headers
      it_rows       = lt_rows
      iv_sheet_name = |DPR Summary { iv_date_from DATE = RAW } to { iv_date_to DATE = RAW }| ).
  ENDMETHOD.
